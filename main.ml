  ...
  then 
    try print_string("Game Over \n Your score: " ^ 
      ...
      |"y" -> print_string "What size board would you like? \n";
        ...
        playing_game time (State.init (Board.generate (Standard(int_of_string (read_line ()))))) []
      |"n" -> ()
...
let rec main () =
  ...
  try
    ...
    match  read_line () with
    |"s" -> playing_game (Unix.time() +. 90.) (State.init (Board.generate (Standard side))) []
    |"r" -> playing_game (Unix.time() +. 90.) (State.init (Board.generate (Random side))) []
